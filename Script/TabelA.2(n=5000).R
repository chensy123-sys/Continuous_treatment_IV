...
load("data/LLKR_fit_3.RData", envir = env1)
load("data/LLKR_fit_4.RData", envir = env2)
...
set.seed(2025)
A_test_2 <- sim_data(1e4, theta_func=theta) %>% select(A) %>% filter(A >= -0.75 & A <= -0.25)
...
colnames(my_tab) <- c('bias_-0.4','bias_-0.5','bias_-0.6','bias_N', 'rmse_-0.4','rmse_-0.5','rmse_-0.6','rmse_N')
...
write.csv(my_tab, file = 'data/TableA.2(n=5000).csv')
