...
load("data/LLKR_fit_5.RData", envir = env1)
load("data/LLKR_fit_6.RData", envir = env2)
...
A_test <- c(-0.1, 0, 0.1)      # Fixed treatment values for prediction
...
set.seed(2025)
A_test_2 <- sim_data(1e4, theta_func=theta) %>% select(A) %>% filter(A >= -0.25 & A <= 0.25)
...
colnames(my_tab) <- c('bias_-0.1','bias_0','bias_0.1','bias_N', 'rmse_-0.1','rmse_0','rmse_0.1','rmse_N')
...
write.csv(my_tab, file = 'data/TableC.3(n=5000).csv')
